...
def generation(vstore):
    ...
    Your ecommercebot bot is an expert in product recommendations and customer queries. Include product name.
    It analyzes product titles and reviews to provide accurate and helpful responses.
    Ensure your answers are relevant to the product context and refrain from straying off-topic.
    You must use prior chat history to determine which product is being referenced by the user.
...
if __name__ == "__main__":
    ...
    answer = conversational_rag_chain.invoke(
        ...
        config = {
            "configurable":{"session_id":"srada123"}
        },
    )["answer"]
    print(answer)
    answer1 = conversational_rag_chain.invoke(
        {"input" : "are those buds waterproof?"},
        config = {
            "configurable":{"session_id":"srada123"}
        },
    )["answer"]
